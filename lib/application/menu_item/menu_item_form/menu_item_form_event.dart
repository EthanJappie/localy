...
abstract class MenuItemFormEvent with _$MenuItemFormEvent {
  const factory MenuItemFormEvent.initialized(Option<MenuItem> initialMenuItemOption) =
  _Initialized;

  const factory MenuItemFormEvent.saved(String menuID) = _Saved;

  const factory MenuItemFormEvent.menuItemNameChanged(String name) = _MenuItemNameChanged;

  const factory MenuItemFormEvent.menuItemDescriptionChanged(String description) =
  _MenuItemNotesChanged;

  const factory MenuItemFormEvent.sequenceOfAppearanceChanged(
      int sequenceOfAppearance) = _SequenceOfAppearanceChanged;

  const factory MenuItemFormEvent.priceChanged(
