import 'package:flutter/material.dart';
import 'package:adobe_xd/pinned.dart';
import 'package:flutter_svg/flutter_svg.dart';

class Menugreennotlogged extends StatelessWidget {
  const Menugreennotlogged({
    Key key,
  }) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xff34c47c),
      body: Stack(
        children: <Widget>[
          // Adobe XD layer: 'Bg' (shape)
          Container(
            width: 375.0,
            height: 812.0,
            decoration: const BoxDecoration(
              color: Color(0xff34c47c),
            ),
          ),
          Transform.translate(
            offset: const Offset(20.0, 15.0),
            child:
                // Adobe XD layer: 'iOS elements' (group)
                SizedBox(
              width: 340.0,
              height: 788.0,
              child: Stack(
                children: <Widget>[
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(0.0, 0.0, 340.3, 16.0),
                    size: const Size(340.3, 788.0),
                    pinLeft: true,
                    pinRight: true,
                    pinTop: true,
                    fixedHeight: true,
                    child: Stack(
                      children: <Widget>[
                        Pinned.fromSize(
                          bounds: const Rect.fromLTWH(316.0, 2.3, 22.0, 11.3),
                          size: const Size(340.3, 16.0),
                          pinRight: true,
                          pinTop: true,
                          pinBottom: true,
                          fixedWidth: true,
                          child:
                              // Adobe XD layer: 'Border' (group)
                              Stack(
                            children: <Widget>[
                              Pinned.fromSize(
                                bounds: const Rect.fromLTWH(0.0, 0.0, 22.0, 11.3),
                                size: const Size(22.0, 11.3),
                                pinLeft: true,
                                pinRight: true,
                                pinTop: true,
                                pinBottom: true,
                                child:
                                    // Adobe XD layer: 'Fill' (shape)
                                    SvgPicture.string(
                                  _svg_ssip0,
                                  allowDrawingOutsideViewBox: true,
                                  fit: BoxFit.fill,
                                ),
                              ),
                              Pinned.fromSize(
                                bounds: const Rect.fromLTWH(0.0, 0.0, 22.0, 11.3),
                                size: const Size(22.0, 11.3),
                                pinLeft: true,
                                pinRight: true,
                                pinTop: true,
                                pinBottom: true,
                                child:
                                    // Adobe XD layer: 'Shape' (shape)
                                    SvgPicture.string(
                                  _svg_4798hn,
                                  allowDrawingOutsideViewBox: true,
                                  fit: BoxFit.fill,
                                ),
                              ),
                            ],
                          ),
                        ),
                        Pinned.fromSize(
                          bounds: const Rect.fromLTWH(339.0, 6.0, 1.3, 4.0),
                          size: const Size(340.3, 16.0),
                          pinRight: true,
                          fixedWidth: true,
                          fixedHeight: true,
                          child:
                              // Adobe XD layer: 'Cap' (group)
                              Stack(
                            children: <Widget>[
                              Pinned.fromSize(
                                bounds: const Rect.fromLTWH(-5.0, -5.0, 11.3, 14.0),
                                size: const Size(1.3, 4.0),
                                pinLeft: true,
                                pinRight: true,
                                pinTop: true,
                                pinBottom: true,
                                child:
                                    // Adobe XD layer: 'Fill' (shape)
                                    SvgPicture.string(
                                  _svg_bbjvt8,
                                  allowDrawingOutsideViewBox: true,
                                  fit: BoxFit.fill,
                                ),
                              ),
                              Pinned.fromSize(
                                bounds: const Rect.fromLTWH(0.0, 0.0, 1.3, 4.0),
                                size: const Size(1.3, 4.0),
                                pinLeft: true,
                                pinRight: true,
                                pinTop: true,
                                pinBottom: true,
                                child:
                                    // Adobe XD layer: 'Shape' (shape)
                                    SvgPicture.string(
                                  _svg_v5x261,
                                  allowDrawingOutsideViewBox: true,
                                  fit: BoxFit.fill,
                                ),
                              ),
                            ],
                          ),
                        ),
                        Pinned.fromSize(
                          bounds: const Rect.fromLTWH(318.0, 4.3, 18.0, 7.3),
                          size: const Size(340.3, 16.0),
                          pinRight: true,
                          fixedWidth: true,
                          fixedHeight: true,
                          child:
                              // Adobe XD layer: 'Capacity' (group)
                              Stack(
                            children: <Widget>[
                              Pinned.fromSize(
                                bounds: const Rect.fromLTWH(-5.0, -5.0, 28.0, 17.3),
                                size: const Size(18.0, 7.3),
                                pinLeft: true,
                                pinRight: true,
                                pinTop: true,
                                pinBottom: true,
                                child:
                                    // Adobe XD layer: 'Fill' (shape)
                                    SvgPicture.string(
                                  _svg_oyrqs2,
                                  allowDrawingOutsideViewBox: true,
                                  fit: BoxFit.fill,
                                ),
                              ),
                              Pinned.fromSize(
                                bounds: const Rect.fromLTWH(0.0, 0.0, 18.0, 7.3),
                                size: const Size(18.0, 7.3),
                                pinLeft: true,
                                pinRight: true,
                                pinTop: true,
                                pinBottom: true,
                                child:
                                    // Adobe XD layer: 'Shape' (shape)
                                    SvgPicture.string(
                                  _svg_3fhxp,
                                  allowDrawingOutsideViewBox: true,
                                  fit: BoxFit.fill,
                                ),
                              ),
                            ],
                          ),
                        ),
                        Pinned.fromSize(
                          bounds: const Rect.fromLTWH(295.7, 2.3, 15.3, 11.0),
                          size: const Size(340.3, 16.0),
                          pinRight: true,
                          pinTop: true,
                          fixedWidth: true,
                          fixedHeight: true,
                          child:
                              // Adobe XD layer: 'Wifi' (group)
                              Stack(
                            children: <Widget>[
                              Pinned.fromSize(
                                bounds: const Rect.fromLTWH(-5.0, -5.0, 25.3, 21.0),
                                size: const Size(15.3, 11.0),
                                pinLeft: true,
                                pinRight: true,
                                pinTop: true,
                                pinBottom: true,
                                child:
                                    // Adobe XD layer: 'Fill' (shape)
                                    SvgPicture.string(
                                  _svg_na4jco,
                                  allowDrawingOutsideViewBox: true,
                                  fit: BoxFit.fill,
                                ),
                              ),
                              Pinned.fromSize(
                                bounds: const Rect.fromLTWH(0.0, 0.0, 15.3, 11.0),
                                size: const Size(15.3, 11.0),
                                pinLeft: true,
                                pinRight: true,
                                pinTop: true,
                                pinBottom: true,
                                child:
                                    // Adobe XD layer: 'Shape' (shape)
                                    SvgPicture.string(
                                  _svg_k7pnf6,
                                  allowDrawingOutsideViewBox: true,
                                  fit: BoxFit.fill,
                                ),
                              ),
                            ],
                          ),
                        ),
                        Pinned.fromSize(
                          bounds: const Rect.fromLTWH(273.7, 2.7, 17.0, 10.7),
                          size: const Size(340.3, 16.0),
                          pinRight: true,
                          fixedWidth: true,
                          fixedHeight: true,
                          child:
                              // Adobe XD layer: 'Cellular Connection' (group)
                              Stack(
                            children: <Widget>[
                              Pinned.fromSize(
                                bounds: const Rect.fromLTWH(-5.0, -5.0, 27.0, 20.7),
                                size: const Size(17.0, 10.7),
                                pinLeft: true,
                                pinRight: true,
                                pinTop: true,
                                pinBottom: true,
                                child:
                                    // Adobe XD layer: 'Fill' (shape)
                                    SvgPicture.string(
                                  _svg_19w6dl,
                                  allowDrawingOutsideViewBox: true,
                                  fit: BoxFit.fill,
                                ),
                              ),
                              Pinned.fromSize(
                                bounds: const Rect.fromLTWH(0.0, 0.0, 17.0, 10.7),
                                size: const Size(17.0, 10.7),
                                pinLeft: true,
                                pinRight: true,
                                pinTop: true,
                                pinBottom: true,
                                child:
                                    // Adobe XD layer: 'Shape' (shape)
                                    SvgPicture.string(
                                  _svg_ah8z35,
                                  allowDrawingOutsideViewBox: true,
                                  fit: BoxFit.fill,
                                ),
                              ),
                            ],
                          ),
                        ),
                        Pinned.fromSize(
                          bounds: const Rect.fromLTWH(0.0, 0.0, 54.0, 16.0),
                          size: const Size(340.3, 16.0),
                          pinLeft: true,
                          pinTop: true,
                          pinBottom: true,
                          fixedWidth: true,
                          child:
                              // Adobe XD layer: 'Background' (shape)
                              SvgPicture.string(
                            _svg_yvoya0,
                            allowDrawingOutsideViewBox: true,
                            fit: BoxFit.fill,
                          ),
                        ),
                        Pinned.fromSize(
                          bounds: const Rect.fromLTWH(12.8, 4.9, 27.5, 10.3),
                          size: const Size(340.3, 16.0),
                          pinLeft: true,
                          pinBottom: true,
                          fixedWidth: true,
                          fixedHeight: true,
                          child:
                              // Adobe XD layer: '↳ Time' (shape)
                              SvgPicture.string(
                            _svg_s72f8d,
                            allowDrawingOutsideViewBox: true,
                            fit: BoxFit.fill,
                          ),
                        ),
                      ],
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(101.0, 783.0, 134.0, 5.0),
                    size: const Size(340.3, 788.0),
                    pinBottom: true,
                    fixedWidth: true,
                    fixedHeight: true,
                    child: Container(
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(3.0),
                        color: const Color(0x33ffffff),
                      ),
                    ),
                  ),
                ],
              ),
            ),
          ),
          Transform.translate(
            offset: const Offset(271.0, 89.0),
            child:
                // Adobe XD layer: 'Scaled portview' (group)
                SizedBox(
              width: 150.0,
              height: 634.0,
              child: Stack(
                children: <Widget>[
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(0.0, 0.0, 150.0, 634.0),
                    size: const Size(150.0, 634.0),
                    pinLeft: true,
                    pinRight: true,
                    pinTop: true,
                    pinBottom: true,
                    child: Container(
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(10.0),
                        color: const Color(0xffffffff),
                        boxShadow: const[
                           BoxShadow(
                            color: Color(0x29000000),
                            offset: Offset(0, 20),
                            blurRadius: 40,
                          ),
                        ],
                      ),
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(0.0, 0.0, 150.0, 634.0),
                    size: const Size(150.0, 634.0),
                    pinLeft: true,
                    pinRight: true,
                    pinTop: true,
                    pinBottom: true,
                    child: Container(
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(10.0),
                        color: const Color(0xffffffff),
                      ),
                    ),
                  ),
                ],
              ),
            ),
          ),
          Transform.translate(
            offset: const Offset(40.0, 261.0),
            child:
                // Adobe XD layer: 'Menu links' (group)
                SizedBox(
              width: 151.0,
              height: 292.0,
              child: Stack(
                children: <Widget>[
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(0.0, 192.0, 102.0, 12.0),
                    size: const Size(151.0, 292.0),
                    fixedHeight: true,
                    child:
                        // Adobe XD layer: 'Discounts' (text)
                        const Text(
                      'Terms & Conditions',
                      style: TextStyle(
                        fontFamily: 'SF Pro Text',
                        fontSize: 12,
                        color: Color(0xb2ffffff),
                        letterSpacing: -0.12,
                        height: 1.5,
                      ),
                      textAlign: TextAlign.left,
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(0.0, 236.0, 73.0, 12.0),
                    size: const Size(151.0, 292.0),
                    fixedHeight: true,
                    child:
                        // Adobe XD layer: 'Discounts' (text)
                        const Text(
                      'Privacy Policy',
                      style: TextStyle(
                        fontFamily: 'SF Pro Text',
                        fontSize: 12,
                        color: Color(0xb2ffffff),
                        letterSpacing: -0.12,
                        height: 1.5,
                      ),
                      textAlign: TextAlign.left,
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(0.0, 280.0, 76.0, 12.0),
                    size: const Size(151.0, 292.0),
                    fixedHeight: true,
                    child:
                        // Adobe XD layer: 'Discounts' (text)
                        const Text(
                      'Faq & Support',
                      style: TextStyle(
                        fontFamily: 'SF Pro Text',
                        fontSize: 12,
                        color: Color(0xb2ffffff),
                        letterSpacing: -0.12,
                        height: 1.5,
                      ),
                      textAlign: TextAlign.left,
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(34.0, 46.0, 66.0, 14.0),
                    size: const Size(151.0, 292.0),
                    fixedHeight: true,
                    child:
                        // Adobe XD layer: 'Discounts' (text)
                        const Text(
                      'My orders',
                      style: TextStyle(
                        fontFamily: 'SF Pro Text',
                        fontSize: 14,
                        color: Color(0xffffffff),
                        letterSpacing: -0.14,
                        fontWeight: FontWeight.w700,
                        height: 1.4285714285714286,
                      ),
                      textAlign: TextAlign.left,
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(34.0, 1.0, 117.0, 14.0),
                    size: const Size(151.0, 292.0),
                    fixedHeight: true,
                    child:
                        // Adobe XD layer: 'Discounts' (text)
                        const Text(
                      'Sign in or Sign up',
                      style: TextStyle(
                        fontFamily: 'SF Pro Text',
                        fontSize: 14,
                        color: Color(0xffffffff),
                        letterSpacing: -0.14,
                        fontWeight: FontWeight.w700,
                        height: 1.4285714285714286,
                      ),
                      textAlign: TextAlign.left,
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(34.0, 91.0, 54.0, 14.0),
                    size: const Size(151.0, 292.0),
                    fixedHeight: true,
                    child:
                        // Adobe XD layer: 'Discounts' (text)
                        const Text(
                      'Settings',
                      style: TextStyle(
                        fontFamily: 'SF Pro Text',
                        fontSize: 14,
                        color: Color(0xffffffff),
                        letterSpacing: -0.14,
                        fontWeight: FontWeight.w700,
                        height: 1.4285714285714286,
                      ),
                      textAlign: TextAlign.left,
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(34.0, 136.0, 51.0, 14.0),
                    size: const Size(151.0, 292.0),
                    fixedHeight: true,
                    child:
                        // Adobe XD layer: 'Discounts' (text)
                        const Text(
                      'Contact',
                      style: TextStyle(
                        fontFamily: 'SF Pro Text',
                        fontSize: 14,
                        color: Color(0xffffffff),
                        letterSpacing: -0.14,
                        fontWeight: FontWeight.w700,
                        height: 1.4285714285714286,
                      ),
                      textAlign: TextAlign.left,
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(2.0, 45.0, 11.2, 14.0),
                    size: const Size(151.0, 292.0),
                    child: Stack(
                      children: <Widget>[
                        SvgPicture.string(
                          _svg_e8jafg,
                          allowDrawingOutsideViewBox: true,
                        ),
                      ],
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(0.0, 90.0, 14.0, 14.0),
                    size: const Size(151.0, 292.0),
                    child: Stack(
                      children: <Widget>[
                        SvgPicture.string(
                          _svg_wp73ah,
                          allowDrawingOutsideViewBox: true,
                        ),
                        Transform.translate(
                          offset: const Offset(5.6, 5.7),
                          child: Container(
                            width: 2.8,
                            height: 2.8,
                            decoration: const BoxDecoration(
                              borderRadius: BorderRadius.all(
                                  Radius.elliptical(9999.0, 9999.0)),
                              color: Color(0x80ffffff),
                            ),
                          ),
                        ),
                      ],
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(0.0, 135.0, 14.0, 14.0),
                    size: const Size(151.0, 292.0),
                    child: Stack(
                      children: <Widget>[
                        SvgPicture.string(
                          _svg_dyozmv,
                          allowDrawingOutsideViewBox: true,
                        ),
                      ],
                    ),
                  ),
                  Pinned.fromSize(
                    bounds: const Rect.fromLTWH(1.0, 0.0, 12.6, 14.0),
                    size: const Size(151.0, 292.0),
                    child: Stack(
                      children: <Widget>[
                        SvgPicture.string(
                          _svg_f8wxzv,
                          allowDrawingOutsideViewBox: true,
                        ),
                      ],
                    ),
                  ),
                ],
              ),
            ),
          ),
        ],
      ),
    );
  }
}

const String _svg_ssip0 =
    '<svg viewBox="336.0 17.3 22.0 11.3" ><path transform="translate(336.0, 17.33)" d="M 2.666666984558105 0 L 19.33333015441895 0 L 19.33333015441895 0 C 20.80608940124512 0 22 1.193907022476196 22 2.666666984558105 L 22 8.666666984558105 L 22 8.666666984558105 C 22 10.13943004608154 20.80608940124512 11.33333015441895 19.33333015441895 11.33333015441895 L 2.666666984558105 11.33333015441895 L 2.666666984558105 11.33333015441895 C 1.193907022476196 11.33333015441895 0 10.13943004608154 0 8.666666984558105 L 0 2.666666984558105 L 0 2.666666984558105 C 0 1.193907022476196 1.193907022476196 0 2.666666984558105 0 Z" fill="none" stroke="#ffffff" stroke-width="2" stroke-opacity="0.35" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_4798hn =
    '<svg viewBox="336.0 17.3 22.0 11.3" ><path transform="translate(0.0, 44.0)" d="M 338.6666870117188 -26.66666984558105 L 355.3333129882812 -26.66666984558105 L 355.3333129882812 -26.66666984558105 C 356.8060913085938 -26.66666984558105 358 -25.47275924682617 358 -24 L 358 -18 L 358 -18 C 358 -16.52724075317383 356.8060913085938 -15.33333015441895 355.3333129882812 -15.33333015441895 L 338.6666870117188 -15.33333015441895 L 338.6666870117188 -15.33333015441895 C 337.1939086914062 -15.33333015441895 336 -16.52724075317383 336 -18 L 336 -24 L 336 -24 C 336 -25.47275924682617 337.1939086914062 -26.66666984558105 338.6666870117188 -26.66666984558105 Z" fill="none" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_bbjvt8 =
    '<svg viewBox="354.0 16.0 11.3 14.0" ><path transform="translate(0.0, 44.0)" d="M 354 -28 L 365.3280029296875 -28 L 365.3280029296875 -14 L 354 -14 L 354 -28 Z" fill="#ffffff" fill-opacity="0.4" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_v5x261 =
    '<svg viewBox="359.0 21.0 1.3 4.0" ><path transform="translate(0.0, 44.0)" d="M 359 -23 L 359 -19 C 359.8046875 -19.33877944946289 360.3280029296875 -20.12686920166016 360.3280029296875 -21 C 360.3280029296875 -21.87313079833984 359.8046875 -22.66122055053711 359 -23" fill="#ffffff" fill-opacity="0.4" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_oyrqs2 =
    '<svg viewBox="333.0 14.3 28.0 17.3" ><path transform="translate(0.0, 44.0)" d="M 333 -29.66666984558105 L 361 -29.66666984558105 L 361 -12.33333015441895 L 333 -12.33333015441895 L 333 -29.66666984558105 Z" fill="#ffffff" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_3fhxp =
    '<svg viewBox="338.0 19.3 18.0 7.3" ><path transform="translate(0.0, 44.0)" d="M 339.3333129882812 -24.66666984558105 L 354.6666870117188 -24.66666984558105 L 354.6666870117188 -24.66666984558105 C 355.4030151367188 -24.66666984558105 356 -24.06970977783203 356 -23.33333015441895 L 356 -18.66666984558105 L 356 -18.66666984558105 C 356 -17.93029022216797 355.4030151367188 -17.33333015441895 354.6666870117188 -17.33333015441895 L 339.3333129882812 -17.33333015441895 L 339.3333129882812 -17.33333015441895 C 338.5969848632812 -17.33333015441895 338 -17.93029022216797 338 -18.66666984558105 L 338 -23.33333015441895 L 338 -23.33333015441895 C 338 -24.06970977783203 338.5969848632812 -24.66666984558105 339.3333129882812 -24.66666984558105 Z" fill="#ffffff" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_na4jco =
    '<svg viewBox="310.7 12.3 25.3 21.0" ><path transform="translate(0.0, 44.0)" d="M 310.6936950683594 -31.66933059692383 L 335.9660949707031 -31.66933059692383 L 335.9660949707031 -10.70376014709473 L 310.6936950683594 -10.70376014709473 L 310.6936950683594 -31.66933059692383 Z" fill="#ffffff" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_k7pnf6 =
    '<svg viewBox="315.7 17.3 15.3 11.0" ><path transform="translate(0.0, 44.0)" d="M 323.3302917480469 -24.39200019836426 C 325.5462036132812 -24.39189910888672 327.6773986816406 -23.54047012329102 329.2832946777344 -22.01366996765137 C 329.404296875 -21.89579010009766 329.5975952148438 -21.89727973937988 329.7167053222656 -22.01700019836426 L 330.8727111816406 -23.18367004394531 C 330.9330139160156 -23.2443904876709 330.9666137695312 -23.32663917541504 330.9660949707031 -23.4122200012207 C 330.9656066894531 -23.49780082702637 330.9309997558594 -23.57965087890625 330.8699951171875 -23.63966941833496 C 326.6549072265625 -27.6792106628418 320.0050964355469 -27.6792106628418 315.7900085449219 -23.63966941833496 C 315.7289123535156 -23.5797004699707 315.6943054199219 -23.49786949157715 315.6936950683594 -23.41229057312012 C 315.6932067871094 -23.32670974731445 315.7267150878906 -23.24443054199219 315.7869873046875 -23.18367004394531 L 316.9432983398438 -22.01700019836426 C 317.0624084472656 -21.8971004486084 317.2557983398438 -21.89561080932617 317.376708984375 -22.01366996765137 C 318.9827880859375 -23.54056930541992 321.1141967773438 -24.39200973510742 323.3302917480469 -24.39200019836426 L 323.3302917480469 -24.39200019836426 Z M 323.3302917480469 -20.5963306427002 C 324.5477905273438 -20.59641075134277 325.7218933105469 -20.14388084411621 326.6242980957031 -19.32666969299316 C 326.7463989257812 -19.21068954467773 326.9386901855469 -19.21319961547852 327.0577087402344 -19.33233070373535 L 328.2123107910156 -20.49900054931641 C 328.2731018066406 -20.56019020080566 328.306884765625 -20.64320945739746 328.3059997558594 -20.7294807434082 C 328.3051147460938 -20.81574058532715 328.2697143554688 -20.89805030822754 328.2077026367188 -20.95800018310547 C 325.4595031738281 -23.51437950134277 321.2034912109375 -23.51437950134277 318.4552917480469 -20.95800018310547 C 318.393310546875 -20.89805030822754 318.3577880859375 -20.81570053100586 318.3569946289062 -20.72941017150879 C 318.356201171875 -20.64311981201172 318.3901062011719 -20.56011009216309 318.4509887695312 -20.49900054931641 L 319.6052856445312 -19.33233070373535 C 319.7243041992188 -19.21319961547852 319.9165954589844 -19.21068954467773 320.0386962890625 -19.32666969299316 C 320.9404907226562 -20.14333915710449 322.1135864257812 -20.5958309173584 323.3302917480469 -20.5963306427002 L 323.3302917480469 -20.5963306427002 Z M 325.5492858886719 -17.8120002746582 C 325.6111145019531 -17.87261009216309 325.6451110839844 -17.95601081848145 325.643310546875 -18.04250907897949 C 325.6416015625 -18.12902069091797 325.6041870117188 -18.21096992492676 325.5400085449219 -18.26899909973145 C 324.264404296875 -19.34787940979004 322.3962097167969 -19.34787940979004 321.1206970214844 -18.26899909973145 C 321.056396484375 -18.21100997924805 321.0190124511719 -18.12908935546875 321.0172119140625 -18.04258918762207 C 321.0152893066406 -17.95607948303223 321.0492858886719 -17.87265014648438 321.1109924316406 -17.8120002746582 L 323.1087036132812 -15.79633045196533 C 323.1672058105469 -15.73709011077881 323.2470092773438 -15.70376014709473 323.3302917480469 -15.70376014709473 C 323.4136047363281 -15.70376014709473 323.493408203125 -15.73709011077881 323.552001953125 -15.79633045196533 L 325.5492858886719 -17.8120002746582 Z" fill="#ffffff" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_19w6dl =
    '<svg viewBox="288.7 12.7 27.0 20.7" ><path transform="translate(0.0, 44.0)" d="M 288.6666870117188 -31.33333015441895 L 315.6666870117188 -31.33333015441895 L 315.6666870117188 -10.66666984558105 L 288.6666870117188 -10.66666984558105 L 288.6666870117188 -31.33333015441895 Z" fill="#ffffff" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_ah8z35 =
    '<svg viewBox="293.7 17.7 17.0 10.7" ><path transform="translate(0.0, 44.0)" d="M 294.6666870117188 -19.66666984558105 L 295.6666870117188 -19.66666984558105 C 296.218994140625 -19.66666984558105 296.6666870117188 -19.21895027160645 296.6666870117188 -18.66666984558105 L 296.6666870117188 -16.66666984558105 C 296.6666870117188 -16.1143798828125 296.218994140625 -15.66666984558105 295.6666870117188 -15.66666984558105 L 294.6666870117188 -15.66666984558105 C 294.1144104003906 -15.66666984558105 293.6666870117188 -16.1143798828125 293.6666870117188 -16.66666984558105 L 293.6666870117188 -18.66666984558105 C 293.6666870117188 -19.21895027160645 294.1144104003906 -19.66666984558105 294.6666870117188 -19.66666984558105 L 294.6666870117188 -19.66666984558105 Z M 299.3333129882812 -21.66666984558105 L 300.3333129882812 -21.66666984558105 C 300.8855895996094 -21.66666984558105 301.3333129882812 -21.21895027160645 301.3333129882812 -20.66666984558105 L 301.3333129882812 -16.66666984558105 C 301.3333129882812 -16.1143798828125 300.8855895996094 -15.66666984558105 300.3333129882812 -15.66666984558105 L 299.3333129882812 -15.66666984558105 C 298.781005859375 -15.66666984558105 298.3333129882812 -16.1143798828125 298.3333129882812 -16.66666984558105 L 298.3333129882812 -20.66666984558105 C 298.3333129882812 -21.21895027160645 298.781005859375 -21.66666984558105 299.3333129882812 -21.66666984558105 Z M 304 -24 L 305 -24 C 305.5523071289062 -24 306 -23.55228042602539 306 -23 L 306 -16.66666984558105 C 306 -16.1143798828125 305.5523071289062 -15.66666984558105 305 -15.66666984558105 L 304 -15.66666984558105 C 303.4476928710938 -15.66666984558105 303 -16.1143798828125 303 -16.66666984558105 L 303 -23 C 303 -23.55228042602539 303.4476928710938 -24 304 -24 Z M 308.6666870117188 -26.33333015441895 L 309.6666870117188 -26.33333015441895 C 310.218994140625 -26.33333015441895 310.6666870117188 -25.8856201171875 310.6666870117188 -25.33333015441895 L 310.6666870117188 -16.66666984558105 C 310.6666870117188 -16.1143798828125 310.218994140625 -15.66666984558105 309.6666870117188 -15.66666984558105 L 308.6666870117188 -15.66666984558105 C 308.1144104003906 -15.66666984558105 307.6666870117188 -16.1143798828125 307.6666870117188 -16.66666984558105 L 307.6666870117188 -25.33333015441895 C 307.6666870117188 -25.8856201171875 308.1144104003906 -26.33333015441895 308.6666870117188 -26.33333015441895 L 308.6666870117188 -26.33333015441895 Z" fill="#ffffff" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_yvoya0 =
    '<svg viewBox="20.0 15.0 54.0 16.0" ><path transform="translate(-1.0, 50.0)" d="M 21 -19 L 75 -19 L 75 -35 L 21 -35 L 21 -19 Z" fill="none" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_s72f8d =
    '<svg viewBox="32.8 19.9 27.5 10.3" ><path transform="translate(20.0, 17.0)" d="M 16.43259620666504 13.2392578125 C 18.94138526916504 13.2392578125 20.43161964416504 11.27734375 20.43161964416504 7.955078125 C 20.43161964416504 6.7041015625 20.19236183166504 5.6513671875 19.73435401916504 4.837890625 C 19.07126808166504 3.5732421875 17.92966651916504 2.8896484375 16.48728370666504 2.8896484375 C 14.34079933166504 2.8896484375 12.82322120666504 4.33203125 12.82322120666504 6.35546875 C 12.82322120666504 8.255859375 14.19040870666504 9.63671875 16.07029151916504 9.63671875 C 17.22556495666504 9.63671875 18.16208839416504 9.0966796875 18.62693214416504 8.16015625 L 18.64743995666504 8.16015625 C 18.64743995666504 8.16015625 18.67478370666504 8.16015625 18.68161964416504 8.16015625 C 18.69529151916504 8.16015625 18.74314308166504 8.16015625 18.74314308166504 8.16015625 C 18.74314308166504 10.416015625 17.88865089416504 11.7626953125 16.44626808166504 11.7626953125 C 15.59861183166504 11.7626953125 14.94236183166504 11.2978515625 14.71677589416504 10.552734375 L 12.95993995666504 10.552734375 C 13.25388526916504 12.1728515625 14.62790870666504 13.2392578125 16.43259620666504 13.2392578125 Z M 16.49411964416504 8.2353515625 C 15.35935401916504 8.2353515625 14.55271339416504 7.4287109375 14.55271339416504 6.30078125 C 14.55271339416504 5.2001953125 15.40036964416504 4.359375 16.50095558166504 4.359375 C 17.60154151916504 4.359375 18.44919776916504 5.2138671875 18.44919776916504 6.328125 C 18.44919776916504 7.4287109375 17.62204933166504 8.2353515625 16.49411964416504 8.2353515625 Z M 23.53221893310547 11.9541015625 C 24.18163299560547 11.9541015625 24.62596893310547 11.4892578125 24.62596893310547 10.8740234375 C 24.62596893310547 10.251953125 24.18163299560547 9.7939453125 23.53221893310547 9.7939453125 C 22.88964080810547 9.7939453125 22.43846893310547 10.251953125 22.43846893310547 10.8740234375 C 22.43846893310547 11.4892578125 22.88964080810547 11.9541015625 23.53221893310547 11.9541015625 Z M 23.53221893310547 6.8271484375 C 24.18163299560547 6.8271484375 24.62596893310547 6.369140625 24.62596893310547 5.75390625 C 24.62596893310547 5.1318359375 24.18163299560547 4.673828125 23.53221893310547 4.673828125 C 22.88964080810547 4.673828125 22.43846893310547 5.1318359375 22.43846893310547 5.75390625 C 22.43846893310547 6.369140625 22.88964080810547 6.8271484375 23.53221893310547 6.8271484375 Z M 31.44532012939453 13 L 33.13379669189453 13 L 33.13379669189453 11.1611328125 L 34.46680450439453 11.1611328125 L 34.46680450439453 9.6708984375 L 33.13379669189453 9.6708984375 L 33.13379669189453 3.1357421875 L 30.64551544189453 3.1357421875 C 28.90235137939453 5.7607421875 27.51465606689453 7.955078125 26.62598419189453 9.5888671875 L 26.62598419189453 11.1611328125 L 31.44532012939453 11.1611328125 L 31.44532012939453 13 Z M 28.25977325439453 9.609375 C 29.40821075439453 7.5859375 30.43360137939453 5.9658203125 31.37696075439453 4.5712890625 L 31.47266387939453 4.5712890625 L 31.47266387939453 9.7119140625 L 28.25977325439453 9.7119140625 L 28.25977325439453 9.609375 Z M 38.51076507568359 13 L 40.27443695068359 13 L 40.27443695068359 3.1357421875 L 38.51760101318359 3.1357421875 L 35.94045257568359 4.9404296875 L 35.94045257568359 6.6357421875 L 38.39455413818359 4.9130859375 L 38.51076507568359 4.9130859375 L 38.51076507568359 13 Z" fill="#ffffff" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_e8jafg =
    '<svg viewBox="50.0 354.0 11.2 14.0" ><path transform="translate(50.0, 354.0)" d="M 10.5 14 L 0.699999988079071 14 C 0.2799929976463318 14 0 13.72000122070312 0 13.30000019073486 L 0 0.699999988079071 C 0 0.2799999713897705 0.2799929678440094 0 0.699999988079071 0 L 7.699999332427979 0 C 7.909992218017578 0 8.050000190734863 0.06999999284744263 8.190007209777832 0.2099999934434891 L 10.99000644683838 3.010000228881836 C 11.13001346588135 3.150000333786011 11.19999980926514 3.290000200271606 11.19999980926514 3.5 L 11.19999980926514 13.30000019073486 C 11.19999980926514 13.72000122070312 10.92000675201416 14 10.5 14 Z M 1.399999976158142 12.60000133514404 L 9.800000190734863 12.60000133514404 L 9.800000190734863 3.78000020980835 L 7.420006275177002 1.399999976158142 L 1.399999976158142 1.399999976158142 L 1.399999976158142 12.60000133514404 Z" fill="#ffffff" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /><path transform="translate(47.6, 353.1)" d="M 11.29000949859619 5.308784008026123 L 9.190007209777832 3.208785057067871 C 8.980014801025391 2.99878454208374 8.70000171661377 2.928784847259521 8.420007705688477 3.068784713745117 C 8.140013694763184 3.208784580230713 8 3.418784379959106 8 3.698784589767456 L 8 6.498783588409424 L 10.80000019073486 6.498783588409424 C 11.07999324798584 6.498783588409424 11.35999393463135 6.358785152435303 11.42999362945557 6.078784942626953 C 11.57000732421875 5.798784732818604 11.50000095367432 5.518784523010254 11.29000949859619 5.308784008026123 Z" fill="#ffffff" fill-opacity="0.4" stroke="none" stroke-width="1" stroke-opacity="0.4" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_wp73ah =
    '<svg viewBox="0.0 0.0 14.0 14.0" ><path transform="translate(0.0, 0.0)" d="M 7.693076133728027 14 L 6.306937217712402 14 C 6.029716491699219 14 5.752488613128662 13.79208087921143 5.683181285858154 13.51485157012939 L 5.059426307678223 11.71287059783936 L 3.39606761932373 12.54455375671387 C 3.11884593963623 12.68316841125488 2.841618299484253 12.61386108398438 2.564377069473267 12.40594005584717 L 1.594086885452271 11.43564224243164 C 1.386173009872437 11.22772312164307 1.316866159439087 10.88118839263916 1.455466151237488 10.60396099090576 L 2.287156105041504 8.940593719482422 L 0.4851554036140442 8.316832542419434 C 0.207934632897377 8.247525215148926 0 7.970296382904053 0 7.693068981170654 L 0 6.306930541992188 C 0 6.029703140258789 0.2079138308763504 5.752474784851074 0.4851554036140442 5.683167934417725 L 2.287142515182495 5.05940580368042 L 1.4554523229599 3.396039962768555 C 1.316831588745117 3.11881160736084 1.386138439178467 2.772277593612671 1.594073057174683 2.564356565475464 L 2.5643630027771 1.594059705734253 C 2.772277116775513 1.386131644248962 3.11881160736084 1.316824674606323 3.396053791046143 1.455445408821106 L 5.059412479400635 2.287128686904907 L 5.683167934417725 0.4851485192775726 C 5.752481460571289 0.2079208493232727 6.029702186584473 5.960464122267695e-08 6.306923389434814 5.960464122267695e-08 L 7.693062782287598 5.960464122267695e-08 C 7.970296382904053 1.829449303158981e-08 8.247516632080078 0.2079208195209503 8.316832542419434 0.4851484596729279 L 8.940586090087891 2.287128686904907 L 10.60394668579102 1.455445408821106 C 10.88117408752441 1.316831707954407 11.227707862854 1.386138558387756 11.43564224243164 1.594059348106384 L 12.40593338012695 2.564356327056885 C 12.61384677886963 2.772276878356934 12.68315410614014 3.11881160736084 12.54455375671387 3.396039485931396 L 11.71286487579346 5.059405326843262 L 13.51485157012939 5.683167934417725 C 13.79207229614258 5.752474784851074 14.00000762939453 6.029702186584473 14.00000762939453 6.306930065155029 L 14.00000762939453 7.693068027496338 C 14.00000762939453 7.970295429229736 13.79209327697754 8.247525215148926 13.51485157012939 8.316831588745117 L 11.71286487579346 8.940592765808105 L 12.54455375671387 10.60396003723145 C 12.6831750869751 10.88118743896484 12.61386775970459 11.22772121429443 12.40593338012695 11.43564224243164 L 11.43564224243164 12.40594005584717 C 11.22772979736328 12.61386013031006 10.88119411468506 12.68316745758057 10.60395240783691 12.54455375671387 L 8.940593719482422 11.71287059783936 L 8.316832542419434 13.51485157012939 C 8.247516632080078 13.79207229614258 7.970296382904053 14 7.693076133728027 14 Z M 6.792093276977539 12.61386108398438 L 7.207942008972168 12.61386108398438 L 7.8316969871521 10.74257469177246 C 7.970318794250488 10.32673263549805 8.663386344909668 10.04950523376465 9.009921073913574 10.25742530822754 L 10.81190967559814 11.15841579437256 L 11.08912944793701 10.88118934631348 L 10.18814659118652 9.079208374023438 C 9.980233192443848 8.663366317749023 10.25745964050293 8.039604187011719 10.67330265045166 7.900989055633545 L 12.54459571838379 7.277227401733398 L 12.54459571838379 6.861386299133301 L 10.67330265045166 6.23762321472168 C 10.25745391845703 6.099010467529297 9.980233192443848 5.405941486358643 10.18814659118652 5.059406280517578 L 11.08912944793701 3.257426261901855 L 10.81190967559814 2.980198621749878 L 9.009921073913574 3.881188631057739 C 8.594074249267578 4.089109420776367 7.970318794250488 3.811881542205811 7.8316969871521 3.396040916442871 L 7.207942008972168 1.524753570556641 L 6.792093276977539 1.524753570556641 L 6.168337345123291 3.396040916442871 C 6.029716491699219 3.811881542205811 5.336647510528564 4.089109420776367 4.990112781524658 3.881188631057739 L 3.188125610351562 2.980198621749878 L 2.910905122756958 3.257426261901855 L 3.811887979507446 5.059406280517578 C 4.019802093505859 5.475248336791992 3.742574214935303 6.099010467529297 3.326733589172363 6.23762321472168 L 1.45543897151947 6.861386299133301 L 1.45543897151947 7.277227401733398 L 3.326733589172363 7.900989055633545 C 3.742581367492676 8.039604187011719 4.019802093505859 8.732672691345215 3.811887979507446 9.079208374023438 L 2.910905122756958 10.88118934631348 L 3.188125610351562 11.15841579437256 L 4.990112781524658 10.25742530822754 C 5.405961513519287 10.04950428009033 6.029716491699219 10.32673263549805 6.168337345123291 10.74257469177246 L 6.792093276977539 12.61386108398438 Z" fill="#ffffff" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_dyozmv =
    '<svg viewBox="0.0 0.0 14.0 14.0" ><path  d="M 6.509998798370361 13.78999423980713 L 3.91999888420105 11.19999694824219 L 0.6999998092651367 11.19999694824219 C 0.2799928784370422 11.19999694824219 0 10.92000293731689 0 10.49999523162842 L 0 0.6999998092651367 C 0 0.2799928784370422 0.2799928784370422 0 0.6999998092651367 0 L 13.29999446868896 0 C 13.72000122070312 0 13.9999942779541 0.2799928784370422 13.9999942779541 0.6999998092651367 L 13.9999942779541 10.49999523162842 C 13.9999942779541 10.92000293731689 13.72000122070312 11.19999694824219 13.29999446868896 11.19999694824219 L 10.07998847961426 11.19999694824219 L 7.489990711212158 13.78999423980713 C 7.209997177124023 14.07000160217285 6.789998054504395 14.07000160217285 6.509998798370361 13.78999423980713 Z M 1.399999618530273 9.799995422363281 L 4.19999885559082 9.799995422363281 C 4.409997940063477 9.799995422363281 4.549998760223389 9.870003700256348 4.689997673034668 10.00999641418457 L 6.999997138977051 12.31999588012695 L 9.309996604919434 10.00999641418457 C 9.449996948242188 9.869996070861816 9.58998966217041 9.799995422363281 9.799995422363281 9.799995422363281 L 12.59999465942383 9.799995422363281 L 12.59999465942383 1.399999618530273 L 1.399999618530273 1.399999618530273 L 1.399999618530273 9.799995422363281 Z" fill="#ffffff" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /><path transform="translate(-1.5, -1.5)" d="M 11.29999732971191 6.399999141693115 L 5.699999809265137 6.399999141693115 C 5.279992580413818 6.399999141693115 5 6.120006561279297 5 5.699999809265137 C 5 5.279992580413818 5.279992580413818 5 5.699999809265137 5 L 11.29999732971191 5 C 11.72000408172607 5 11.99999713897705 5.279992580413818 11.99999713897705 5.699999809265137 C 11.99999713897705 6.120006561279297 11.72000408172607 6.399999141693115 11.29999732971191 6.399999141693115 Z" fill="#ffffff" fill-opacity="0.5" stroke="none" stroke-width="1" stroke-opacity="0.5" stroke-miterlimit="4" stroke-linecap="butt" /><path transform="translate(-1.5, -2.7)" d="M 11.29999732971191 10.40000057220459 L 5.699999809265137 10.40000057220459 C 5.279992580413818 10.40000057220459 5 10.12000751495361 5 9.70000171661377 C 5 9.279993057250977 5.279992580413818 9 5.699999809265137 9 L 11.29999732971191 9 C 11.72000408172607 9 11.99999713897705 9.279993057250977 11.99999713897705 9.70000171661377 C 11.99999713897705 10.12000751495361 11.72000408172607 10.40000057220459 11.29999732971191 10.40000057220459 Z" fill="#ffffff" fill-opacity="0.5" stroke="none" stroke-width="1" stroke-opacity="0.5" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_f8wxzv =
    '<svg viewBox="42.0 208.0 12.6 14.0" ><path transform="translate(38.4, 208.0)" d="M 16.19999885559082 0.699999988079071 L 16.19999885559082 13.30000019073486 C 16.19999885559082 13.72000598907471 15.92000102996826 14 15.5 14 L 12 14 L 12 12.60000133514404 L 14.80000114440918 12.60000133514404 L 14.80000114440918 1.399999976158142 L 12 1.399999976158142 L 12 0 L 15.5 0 C 15.92000102996826 0 16.19999885559082 0.2799929678440094 16.19999885559082 0.699999988079071 Z" fill="#ffffff" fill-opacity="0.5" stroke="none" stroke-width="1" stroke-opacity="0.5" stroke-miterlimit="4" stroke-linecap="butt" /><path transform="translate(42.0, 206.8)" d="M 4.409999847412109 4.209989070892334 C 4.12999963760376 4.489982128143311 4.12999963760376 4.909988880157471 4.409999847412109 5.189981937408447 L 6.71999979019165 7.499975204467773 L 0.699999988079071 7.499975204467773 C 0.2799999713897705 7.499975204467773 0 7.77996826171875 0 8.199974060058594 C 0 8.619980812072754 0.2799999713897705 8.89997386932373 0.699999988079071 8.89997386932373 L 6.71999979019165 8.89997386932373 L 4.409999847412109 11.20996761322021 C 4.12999963760376 11.48996067047119 4.12999963760376 11.90996646881104 4.409999847412109 12.18995952606201 C 4.690000057220459 12.4699535369873 5.110000610351562 12.46995258331299 5.390000343322754 12.18995952606201 L 8.889999389648438 8.689960479736328 C 9.170001029968262 8.409967422485352 9.170001029968262 7.989960670471191 8.889999389648438 7.709967136383057 L 5.390000343322754 4.209968090057373 C 5.110000610351562 3.929988861083984 4.689999580383301 3.929988861083984 4.409999847412109 4.209989070892334 Z" fill="#ffffff" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
